...
void pickCallback(const geometry_msgs::Point::ConstPtr &msg) {
  ...
  ROS_INFO("[auto_pick] Pick ROUTINE FINISHED!!!");
}
...
bool moveToStartPosition() {
  ...
  if (arm_ptr->plan(plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS &&
      arm_ptr->execute(plan)) {
    ROS_INFO("[startup] Ready.");
    return true;
  } else {
    ROS_WARN("[startup] Failed to reach start position.");
    return false;
  }
}
